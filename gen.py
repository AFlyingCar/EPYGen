...
{0}    {1}{2} = getattr(__LIBRARY_{3}, "{2}")
{0}except AttributeError as e:
{0}    print(str(e), file=stderr)
{0}    {1}{2} = None
"""
...
def createPyClass(klass, epy):
    ...
    for c in klass.ctors:
        ctor_name = "__pywrapped_{0}_Create{1}".format(klass.name, str(ctor_count) if ctor_count > 0 else "")
...
def createCPPClass(klass, epy, reffed_throws):
    ...
    if len(klass.virtual_funcs) > 0:
        wrapped_name = "__pywrapped_" + class_name
